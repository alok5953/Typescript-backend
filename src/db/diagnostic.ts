...
const $DEV = APP_CONFIG.ENV !== "production"

// Log file for database-related errors:
const logFile = path.join(path.resolve(), "constants", "database_errors.log")
...
pgMonitor.setLog((msg, info) => {
    ...
    if (info.event === "error") {
        ...
        if (info.time) {
            ...
            logText = os.EOL + `${new Date()} ` + logText // add another line break in front;
        }
